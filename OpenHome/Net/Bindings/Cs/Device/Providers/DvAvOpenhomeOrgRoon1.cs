...
namespace OpenHome.Net.Device.Providers
{
    ...
    public class DvProviderAvOpenhomeOrgRoon1 : DvProvider, IDisposable, IDvProviderAvOpenhomeOrgRoon1
    {
        ...
        private static int DoPlayPause(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", new object[] { "PlayPause" }));
                return -1;
            }
            catch (Exception e)
            {
                System.Diagnostics.Debug.WriteLine("WARNING: unexpected exception {0} thrown by {1}", new object[] { e, "PlayPause" });
                System.Diagnostics.Debug.WriteLine("         Only ActionError or PropertyUpdateError should be thrown by actions");
                return -1;
            }
            try
