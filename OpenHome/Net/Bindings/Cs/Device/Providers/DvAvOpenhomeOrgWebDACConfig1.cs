...
namespace OpenHome.Net.Device.Providers
{
    ...
    public class DvProviderAvOpenhomeOrgWebDACConfig1 : DvProvider, IDisposable, IDvProviderAvOpenhomeOrgWebDACConfig1
    {
        ...
        private static int DoGetDACConfig(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", new object[] { "GetDACConfig" }));
                return -1;
            }
            catch (Exception e)
            {
                System.Diagnostics.Debug.WriteLine("WARNING: unexpected exception {0} thrown by {1}", new object[] { e, "GetDACConfig" });
        ...
        private static int DoSetDACConfig(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", new object[] { "SetDACConfig" }));
                return -1;
            }
            catch (Exception e)
            {
                System.Diagnostics.Debug.WriteLine("WARNING: unexpected exception {0} thrown by {1}", new object[] { e, "SetDACConfig" });
                System.Diagnostics.Debug.WriteLine("         Only ActionError or PropertyUpdateError should be thrown by actions");
                return -1;
            }
            try
