...
{
    public interface IDvProviderAvOpenhomeOrgMusicStation1 : IDisposable
    {
        ...
        /// <param name="aValue">New value for the property</param>
        /// <returns>true if the value has been updated; false if aValue was the same as the previous value</returns>
        bool SetPropertyThumbRating(string aValue);

        /// <summary>
        /// Get a copy of the value of the ThumbRating property
        /// </summary>
        /// <returns>Value of the ThumbRating property.</param>
    ...
    /// <summary>
    /// Provider for the av.openhome.org:MusicStation:1 UPnP service
    /// </summary>
    public class DvProviderAvOpenhomeOrgMusicStation1 : DvProvider, IDisposable, IDvProviderAvOpenhomeOrgMusicStation1
    {
        ...
        private ActionDelegate iDelegateSetThumbRating;
        ...
        protected DvProviderAvOpenhomeOrgMusicStation1(DvDevice aDevice)
            : base(aDevice, "av.openhome.org", "MusicStation", 1)
        {
        ...
        /// Set the value of the ThumbRating property
        /// </summary>
        /// <remarks>Can only be called if EnablePropertyThumbRating has previously been called.</remarks>
        /// <param name="aValue">New value for the property</param>
        /// <returns>true if the value has been updated; false if aValue was the same as the previous value</returns>
        public bool SetPropertyThumbRating(string aValue)
        {
            if (iPropertyThumbRating == null)
                throw new PropertyDisabledError();
        ...
        public string PropertyThumbRating()
        {
            if (iPropertyThumbRating == null)
                throw new PropertyDisabledError();
            return iPropertyThumbRating.Value();
        }
        ...
        /// </summary>
        /// <remarks>The action's availability will be published in the device's service.xml.
        /// SetThumbRating must be overridden if this is called.</remarks>
        protected void EnableActionSetThumbRating()
        {
            OpenHome.Net.Core.Action action = new OpenHome.Net.Core.Action("SetThumbRating");
            action.AddInputParameter(new ParameterRelated("ThumbRating", iPropertyThumbRating));
            iDelegateSetThumbRating = new ActionDelegate(DoSetThumbRating);
            EnableAction(action, iDelegateSetThumbRating, GCHandle.ToIntPtr(iGch));
        }
        ...
        ///
        /// Must be implemented iff EnableActionSetThumbRating was called.</remarks>
        /// <param name="aInvocation">Interface allowing querying of aspects of this particular action invocation.</param>
        /// <param name="aThumbRating"></param>
        protected virtual void SetThumbRating(IDvInvocation aInvocation, string aThumbRating)
        {
            throw (new ActionDisabledError());
        }
        ...
        private static int DoPlay(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            DvProviderAvOpenhomeOrgMusicStation1 self = (DvProviderAvOpenhomeOrgMusicStation1)gch.Target;
        ...
        private static int DoSetThumbRating(IntPtr aPtr, IntPtr aInvocation)
        {
            GCHandle gch = GCHandle.FromIntPtr(aPtr);
            DvProviderAvOpenhomeOrgMusicStation1 self = (DvProviderAvOpenhomeOrgMusicStation1)gch.Target;
            DvInvocation invocation = new DvInvocation(aInvocation);
            ...
            try
            {
                ...
                invocation.ReadEnd();
                self.SetThumbRating(invocation, thumbRating);
            }
            catch (ActionError e)
            {
                invocation.ReportActionError(e, "SetThumbRating");
                return -1;
            }
            catch (PropertyUpdateError)
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", new object[] { "SetThumbRating" }));
                return -1;
            }
            catch (Exception e)
            {
                System.Diagnostics.Debug.WriteLine("WARNING: unexpected exception {0} thrown by {1}", new object[] { e, "SetThumbRating" });
                System.Diagnostics.Debug.WriteLine("         Only ActionError or PropertyUpdateError should be thrown by actions");
                return -1;
            }
            try
