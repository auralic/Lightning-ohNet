...
namespace OpenHome.Net.Device.Providers
{
    ...
    public class DvProviderAvOpenhomeOrgWebClockConfig1 : DvProvider, IDisposable, IDvProviderAvOpenhomeOrgWebClockConfig1
    {
        ...
        private static int DoGetClockConfig(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", "GetClockConfig"));
                return -1;
            }
            catch (Exception e)
            {
                Console.WriteLine("WARNING: unexpected exception {0}(\"{1}\") thrown by {2} in {3}", e.GetType(), e.Message, "GetClockConfig", e.TargetSite.Name);
            ...
            {
                Console.WriteLine("ERROR: unexpected exception {0}(\"{1}\") thrown by {2} in {3}", e.GetType(), e.Message, "GetClockConfig", e.TargetSite.Name);
        ...
        private static int DoSetClockConfig(IntPtr aPtr, IntPtr aInvocation)
        {
            ...
            {
                invocation.ReportError(501, String.Format("Invalid value for property {0}", "SetClockConfig"));
                return -1;
            }
            catch (Exception e)
            {
                Console.WriteLine("WARNING: unexpected exception {0}(\"{1}\") thrown by {2} in {3}", e.GetType(), e.Message, "SetClockConfig", e.TargetSite.Name);
                Console.WriteLine("         Only ActionError or PropertyUpdateError should be thrown by actions");
                return -1;
            }
            try
            ...
            {
                Console.WriteLine("ERROR: unexpected exception {0}(\"{1}\") thrown by {2} in {3}", e.GetType(), e.Message, "SetClockConfig", e.TargetSite.Name);
                Console.WriteLine("       Only ActionError can be thrown by action response writer");
            }
            return 0;
        }
