...
class SyncSetGroupStatusAvOpenhomeOrgGroupConfig1 extends SyncProxyAction
{
    private CpProxyAvOpenhomeOrgGroupConfig1 iService;
...
public class CpProxyAvOpenhomeOrgGroupConfig1 extends CpProxy implements ICpProxyAvOpenhomeOrgGroupConfig1
{
    ...
    public void syncSetGroupStatus(String aGroupStatus)
    {
        ...
        beginSetGroupStatus(aGroupStatus, sync.getListener());
        sync.waitToComplete();
        sync.reportError();
    }
     ...
     * Returns immediately and will run the client-specified callback when the
     * action later completes.  Any output arguments can then be retrieved by
     * calling {@link #endSetGroupStatus}.
     * 
     * @param aGroupStatus
     * @param aCallback listener to call back when action completes.
     *                  This is guaranteed to be run but may indicate an error.
    ...
    public void beginSetGroupStatus(String aGroupStatus, ICpProxyListener aCallback)
    {
        Invocation invocation = iService.getInvocation(iActionSetGroupStatus, aCallback);
        int inIndex = 0;
        invocation.addInput(new ArgumentString((ParameterString)iActionSetGroupStatus.getInputParameter(inIndex++), aGroupStatus));
        iService.invokeAction(invocation);
    }
     ...
     * Retrieve the output arguments from an asynchronously invoked action.
     * This may only be called from the callback set in the
     * {@link #beginSetGroupStatus} method.
     *
     * @param aAsyncHandle  argument passed to the delegate set in the
     *          {@link #beginSetGroupStatus} method.
     */
