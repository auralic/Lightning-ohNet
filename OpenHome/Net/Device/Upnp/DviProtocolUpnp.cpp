...
void DviProtocolUpnpDeviceXmlWriter::Write(TIpAddress aAdapter)
{
    ...
    const TChar* passprotect;
    bool protect = false;
    iDeviceUpnp.GetAttribute("Protect", &passprotect);
    if (passprotect!=NULL)
    {
        protect = true;
    }
    ...
    const TChar *enableQPlay;
    iDeviceUpnp.GetAttribute("EnableQPlay",&enableQPlay);
    if(enableQPlay)
    {
        iWriter.Write("<qq:X_QPlay_SoftwareCapability>");
        iWriter.Write("QPlay:2");
        iWriter.Write("</qq:X_QPlay_SoftwareCapability>");
    }
    ...
    if (serviceCount > 0) {
        if(protect)
        {
            iWriter.Write("<protect>");
            iWriter.Write("<password>");
            iWriter.Write(passprotect);
            iWriter.Write("</password>");
        }
